...
import { AlertTriangle, FileText, Package, Search, ShieldCheck, Truck, UserCheck, Banknote, UploadCloud, Sparkles } from "lucide-react";
...
export default function RecordsCertificationGovernancePanel() {
  ...
  const [validation, setValidation] = React.useState([
    { id: "v1", learner: "Amina Osei", program: "Digital Transformation Fundamentals", credential: "DT Strategy Certificate", payment: "Paid", evidence: "Complete", status: "pending-validation" as ValidationStatus, hardCopy: true, reviewer: "Academic Records Administrator", academicStanding: "Verified", financialStanding: "Clear" },
    { id: "v2", learner: "Noah Mensah", program: "AI & Automation in the Workplace", credential: "AI Operations Micro-Credential", payment: "Paid", evidence: "Complete", status: "validated" as ValidationStatus, hardCopy: false, reviewer: "Compliance Owner", academicStanding: "Verified", financialStanding: "Clear" },
    { id: "v3", learner: "Irene Mwangi", program: "Leadership in the Digital Age", credential: "Executive Change Leadership Badge", payment: "Pending", evidence: "Needs review", status: "blocked" as ValidationStatus, hardCopy: true, reviewer: "Academic Records Administrator", academicStanding: "Review Required", financialStanding: "Payment Pending" },
  ]);
  const [certificates, setCertificates] = React.useState([
    { id: "c1", learner: "Amina Osei", credential: "DT Strategy Certificate", number: "DTMA-2026-00141", status: "pending" as CertificateStatus, mode: "Digital + hard copy", action: "Awaiting official validation", source: "Internal (DQ)" },
    { id: "c2", learner: "Noah Mensah", credential: "AI Operations Micro-Credential", number: "DTMA-2026-00119", status: "issued" as CertificateStatus, mode: "Digital", action: "Issued after validation and synced to learner view", source: "Partner (Harvard)" },
    { id: "c3", learner: "Irene Mwangi", credential: "Executive Change Leadership Badge", number: "DTMA-2026-00124", status: "blocked" as CertificateStatus, mode: "Digital + hard copy", action: "Blocked until payment and evidence reconcile", source: "Internal (DQ)" },
  ]);
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-white/80">
              ...
              Stage 4 records workspace
            </div>
            <div>
              <h1 className="text-3xl font-semibold tracking-tight">Records, Certification &amp; Assessment Governance</h1>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-white/75">Official academic outcomes, governed certificate actions, and physical hard-copy fulfillment are managed here so learner-facing progress never becomes an official record without the right controls.</p>
            </div>
          </div>
          <div className="relative w-full max-w-md"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-white/50" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search records, certificate numbers, or fulfillment items" className="border-white/20 bg-white/10 text-white placeholder:text-white/40 pl-9" /></div>
        </div>
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <Card className="border-white/10 bg-white/5 backdrop-blur text-white"><CardContent className="p-5"><ShieldCheck className="mb-3 h-5 w-5 text-[#ff6b4d]" /><div className="text-3xl font-semibold">{validated}</div><div className="text-sm text-white/65">Validated outcomes</div></CardContent></Card>
          <Card className="border-white/10 bg-white/5 backdrop-blur text-white"><CardContent className="p-5"><FileText className="mb-3 h-5 w-5 text-[#ff6b4d]" /><div className="text-3xl font-semibold">{readyToIssue}</div><div className="text-sm text-white/65">Issue-ready credentials</div></CardContent></Card>
          <Card className="border-white/10 bg-white/5 backdrop-blur text-white"><CardContent className="p-5"><Truck className="mb-3 h-5 w-5 text-[#ff6b4d]" /><div className="text-3xl font-semibold">{backlog}</div><div className="text-sm text-white/65">Hard-copy backlog</div></CardContent></Card>
          <Card className="border-white/10 bg-white/5 backdrop-blur text-white"><CardContent className="p-5"><Package className="mb-3 h-5 w-5 text-[#ff6b4d]" /><div className="text-3xl font-semibold">67%</div><div className="text-sm text-white/65">Inventory above reorder point</div></CardContent></Card>
        </div>
      </div>
      ...
      <Tabs defaultValue="validation" className="space-y-6">
        ...
        <TabsContent value="search" className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
          ...
          <Card className="border-slate-200"><CardHeader><CardTitle>Records audit trail</CardTitle><CardDescription>Every material intervention on an official outcome stays visible here.</CardDescription></CardHeader><CardContent className="space-y-3">{audit.map((entry) => <div key={entry.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><div className="font-medium text-slate-900">{entry.action}</div><div className="text-sm text-slate-600">{entry.detail}</div><div className="mt-1 text-xs text-slate-500">{relativeTime(entry.at)}</div></div></div></div>)}<div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">Official records remain durable across curriculum changes, and every correction or override is logged for compliance.</div></CardContent></Card>
        </TabsContent>
      </Tabs>
    </div>
  );
