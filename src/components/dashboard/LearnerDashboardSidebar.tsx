...
const NAV_SECTIONS: NavSection[] = [
  ...
  },
  {
    ...
    label: "Workspaces",
    items: [
      { id: "gamification", label: "Microlearning Paths" },
      { id: "assignments", label: "Assignments" },
      { id: "certificates", label: "Certificates & Badges" },
      { id: "collaboration", label: "Discussions" },
...
const GROUP_ICONS: Partial<Record<LearnerTabId, LucideIcon>> = {
  courses: BookOpen,
...
function groupContainsTab(group: NavGroup, tab: LearnerTabId) {
  return group.items.some((item) => item.id === tab);
}

function sectionContainsTab(section: NavSection, tab: LearnerTabId) {
  if (section.items?.some((item) => item.id === tab)) return true;
  return section.groups?.some((group) => groupContainsTab(group, tab)) ?? false;
}

type LearnerDashboardSidebarProps = {
  activeTab: LearnerTabId;
  onTabChange: (tab: LearnerTabId) => void;
  onSignOut: () => void;
...
export function LearnerDashboardSidebar({
  activeTab,
  onTabChange,
  onSignOut,
  profileName,
  ...
  const renderLeaf = (item: NavLeaf, indented = true) => {
    const isActive = activeTab === item.id;
    const badge =
      item.id === "courses" && inProgressCount > 0 ? inProgressCount :
      item.id === "certificates" && certificateCount > 0 ? certificateCount :
      null;
          ...
          {NAV_SECTIONS.map((section) => (
            <div key={section.id}>
              ...
              {section.items?.map((item) => {
                const Icon = GROUP_ICONS[item.id] ?? LayoutGrid;
                const isActive = activeTab === item.id;
                const badge =
                  item.id === "courses" && inProgressCount > 0 ? inProgressCount :
                  item.id === "certificates" && certificateCount > 0 ? certificateCount :
      ...
      <div className="border-t border-gray-100 p-4">
        <div className="mb-3 flex items-center gap-3 rounded-xl bg-gray-50 p-3">
          ...
          <div className="min-w-0 flex-1">
            <p className={cn(learnerNavSubItemActive, "truncate")}>{profileName || "Learner"}</p>
            <p className={cn(learnerCaption, "truncate")}>{profileEmail}</p>
          </div>
        </div>
        <Button
          variant="ghost"
