...
const ITEM_ICONS: Partial<Record<SMSTabId, LucideIcon>> = {
  courses: BookOpen,
  faculty: Users,
...
type SMSDashboardSidebarProps = {
  activeTab: SMSTabId;
  onTabChange: (tab: SMSTabId) => void;
  onSignOut: () => void;
...
export function SMSDashboardSidebar({
  activeTab,
  onTabChange,
  onSignOut,
  profileName,
  ...
  const billingBadge =
    billingIssueCount > 0 ? billingIssueCount : undefined;

  const renderLeaf = (item: NavLeaf) => {
    const isActive = activeTab === item.id;
    const badge =
      item.id === "billing" && item.label === "Billing" ? billingBadge : item.badge;
          ...
          {NAV_SECTIONS.map((section) => (
            <div key={section.id}>
              ...
              {section.items?.map((item) => {
                ...
                const isActive = activeTab === item.id;
                const badge = item.id === "billing" ? billingBadge : item.badge;
        ...
        <div className="mb-3 flex items-center gap-3 rounded-xl bg-gray-50 p-3">
          ...
          <div className="min-w-0 flex-1">
            <p className={cn(learnerNavSubItemActive, "truncate")}>
              {profileName || "Academy Manager"}
            </p>
            <p className={cn(learnerCaption, "truncate")}>
              {profileEmail || "AMS Portal"}
            </p>
          </div>
        </div>
        <Button
...
export default SMSDashboardSidebar;
